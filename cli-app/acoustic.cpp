...
void generatePianoNote(float* buffer, double freq, bool sustain) {
    ...
    for (int i = 0; i < PIANO_N; i++) {
        ...
        for (int s = 0; s < numStrings; s++) {
            ...
            if (maxH > 30) maxH = 30; // Limit computation
...
static int audioCallback(
    ...
    for (unsigned long i = 0; i < frameCount; i++) {
        ...
        // Drums
        if (snarePH >= 0 && snarePH < SNARE_N) mix += snare[snarePH++];
        if (kickPH >= 0 && kickPH < KICK_N)    mix += kick[kickPH++];
        if (hatPH >= 0 && hatPH < HAT_N)       mix += hihat[hatPH++];
        ...
        delayHead++;
        if (delayHead >= DELAY_LEN) delayHead = 0;
...
// MAIN & INPUT
// =====================
char readChar() { char c; read(STDIN_FILENO, &c, 1); return c; }

void setRawMode(bool enable) {
    static termios oldt; termios newt;
    if (enable) {
...
int main() {
    // Generate Sounds
    generateSnare(); generateKick(); generateHiHat();
    for (int i = 0; i < MAX_PIANO_NOTES; i++) {
        pianoPH[i] = -1;
    ...
    if (deviceId == paNoDevice) {
        for (int i = 0; i < numDevices; i++) {
            ...
            if (info->maxOutputChannels > 0) { deviceId = i; break; }
        }
    }
    ...
    while (true) {
        ...
        if (currentMode == Mode::Drum) {
            ...
            if (c == 'D') { octave = std::max(-2, octave - 1); regeneratePiano(); }
            else if (c == 'C') { octave = std::min(2, octave + 1); regeneratePiano(); }
            
            int n = -1;
            switch(c) {
                case 'a': n=0; break; case 'w': n=1; break; case 's': n=2; break;
                case 'e': n=3; break; case 'd': n=4; break; case 'f': n=5; break;
                case 't': n=6; break; case 'g': n=7; break; case 'y': n=8; break;
                case 'h': n=9; break; case 'u': n=10; break; case 'j': n=11; break;
                case 'k': n=12; break; case 'o': n=13; break; 
                case 'l': n = 14; break; case 'p': n = 15; break;
                case ';': n = 16; break; case '\'': n = 17; break;
                case ']': n = 18; break; case '#': n = 19; break; 
            }
            if (n >= 0) pianoPH[n] = 0;
        }
    }
